...
#include "picam.h" // Princeton Instruments API Header 
#include "stdio.h"
...
//---------------------------------------------------------------------------
//	Princeton Instruments Kinetics Capture example.
//
...
{	
    ...
    if (fwrite(buffer, bytesInReadout, numreadouts, fs) == (pi64u)numreadouts)
        returnOK = true;  
    fclose(fs);

    if (returnOK)
        printf("\n ! ! Successfully wrote %d bytes to %s \n", 
                     (piint)(bytesInReadout * numreadouts), TargetFileName);
    else
...
//---------------------------------------------------------------------------
//	Princeton Instruments Kinetics example.
//
...
void DoKineticsCapture(PicamHandle camera)
{
    ...
    if (err == PicamError_None) 
    {
        printf("\nStart Picam_Acquire():\n  %d frames of Kinetics data (%d pix high), in %d readouts\n", 
            (piint)(framesPerReadout * numReadoutsRequested), kineticsWindowHeight, (piint)numReadoutsRequested);
