...
import numpy as np
from atom.api import Bool, Member, Int
import time
...
class SquareROIAnalysis(ROIAnalysis):
    ...
    def __init__(self, experiment):
        ...
        # initialize with blank arrays, filled in from settings if there
        dtype = [
            ('left', np.uint16),
            ('top', np.uint16),
            ('right', np.uint16),
        ...
        self.shots_path = 'data/'
        self.shots_path += self.experiment.Config.config.get(
            'CAMERA',
            'DataGroup'
        )
        self.shots_path += '/shots'
    ...
    def set_rois(self):
        ...
        if warn:
            ...
            ROIs_bg = np.zeros(
                self.ROI_bg_rows * self.ROI_bg_columns,
                dtype=dtype
            )
    ...
    def analyzeMeasurement(self, measurementResults, iterationResults, experimentResults):
        if self.enable:
            if self.shots_path in measurementResults:
                ...
                # for each image
                measurements = measurementResults[self.shots_path].items()
                for i, (name, shot) in enumerate(measurements):
                    # generate background normalized per pixel
                    bg_per_pix = 0
                    if bg_pixel_cnt != 0:
                        bg_per_pix = np.divide(
                            np.sum(roi_sums(self.ROIs_bg, shot)),
                            bg_pixel_cnt,
                            dtype='float32'
                        )
                    # subtract the average background signal per pixel from
                    # each signal pixel
                    shot_sums = np.subtract(
                        roi_sums(self.ROIs, shot),
                        bg_per_pix * sig_pixel_cnt
                    )
                    # round to nearest integer
                    sum_array[i] = np.rint(shot_sums)

                self.sum_array = sum_array.reshape((
                    num_shots,
                    self.ROI_rows,
                    self.ROI_columns
                ))
