...
class Conex(Prop):
    ...
    def __init__(self, name, experiment, description=''):
        ...
        self.properties += ['SetPos', 'enableVel', 'Vel', "IDString",'Threshold']

        
    def initialize(self):
        logger.debug('CONEX IDString: {}'.format(self.IDString.value))
...
class Conexes(Instrument):
    ...
    def launchServer(self):
        subprocess.Popen(["C:\\Windows\\System32\\cmd.exe","/C","..\\csharp\\CONEX_Server\\bin\\Debug\\CONEX-CC-CSharp-Server.exe"], creationflags=subprocess.CREATE_NEW_CONSOLE)
        ...
        if self.enable:
            ...
            try:
                for i in self.motors:
                    ...
                    loopCount=0
                    loopThreshold=100
                    while(abs(i.SetPos.value - curPos) > i.Threshold.value and loopCount < loopThreshold):  #loop until the error is less than threshold
                        time.sleep(0.1)
                        self.socket.sendmsg("GetPosition")
                        returnedmessage = self.socket.receive()
                        curPos = float(returnedmessage)
