...
class QtField(QtControl, ProxyField):
    ...
    def on_text_edited(self):
        #MTL: editted to prevent error style from being cleared on typing if no validator is in use
        pass
        ...
        d = self.declaration
        if d.validator:
            if not d.validator.validate(self.widget.text()):
    ...
    def set_valid(self, valid):
        print 'qt_field.set_valid: valid='+str(valid)
        if valid:
            self._clear_error_steate()
        else:
            self._set_error_state()
    ...
    def set_text(self, text):
        ...
        if not self._guard & TEXT_GUARD:
            ...
            #MTL: do not clear error state on type, only do that on set_valid
            #self._clear_error_state()
