...
## Initilization
    ...
    # exp.fort_dds.profile(start,'science')
    # exp.mot_3d_dds.profile(t_start,'PGC')
    # #exp.fort_dds.profile(t_start+t_PGC_duration/2,'science') # lowered FORT during PGC
    ...
    # exp.hf_aom_switch.profile(t_start,'on')
    # exp.hf_aom_switch.profile(t_end+0.2,'off') ###
    # # exp.fort_dds.profile(t_end,'science')
    ...
    #
    exp.mot_3d_x_shutter_switch.profile(t_x_shutter_open-6+t_op+t_depump+0.01,'off')
    exp.mot_3d_x_shutter_switch.profile(t_x_shutter_close-6+t_op+t_depump+0.01,'on')
    exp.mot_3d_y_shutter_switch.profile(t_y_shutter_open-6+t_op+t_depump+0.01,'off')
    exp.mot_3d_y_shutter_switch.profile(t_y_shutter_close-6+t_op+t_depump+0.01,'on')
    exp.mot_3d_z1_shutter_switch.profile(t_z1_shutter_open-6+t_op+t_depump+0.01,'off')
    exp.mot_3d_z1_shutter_switch.profile(t_z1_shutter_close-6+t_op+t_depump+0.01,'on')

    ## Science Phase 170 - 175 ms. t_science=170

    # reduce FORT trap depth during science phase
    #t_science2 = t_science+t_op+t_depump
    #exp.fort_dds.profile(t_science2,'science')
    #exp.fort_dds.profile(t_science2+7,'on')
    raman(t_science,t_raman,'r2')
    #FORTdrop(t_science2-0.001, t_FORTdrop)
    ...
    exp.red_pointing_dds.profile(175+t_op+t_depump,'off')
    exp.red_pointing_aom_switch.profile(175+t_op+t_depump,'off')

    ## Blow-away Phase 176ms

    AO(175+t_op+t_depump,0,coil_driver_polarity*-0.1)
    AO(175+t_op+t_depump,1,coil_driver_polarity*0.1)
    AO(175+t_op+t_depump,2, coil_driver_polarity*shimX_BA) #X
    AO(175+t_op+t_depump,3,coil_driver_polarity*shimY_BA) #Y
    AO(175+t_op+t_depump,4,coil_driver_polarity*shimZ_BA) #Z

    t_start=176+t_op+t_depump
    t_end=t_start+t_BA
    ...
    ## Readout Phase
    AO(180+t_op+t_depump,0,0) # turn off quadrupole fields
    AO(180+t_op+t_depump,1,0)
    AO(180+t_op+t_depump,2, coil_driver_polarity*shimX_RO) #X
    AO(180+t_op+t_depump,3, coil_driver_polarity*shimY_RO) #Y
    AO(180+t_op+t_depump,4, coil_driver_polarity*shimZ_RO) #Z
    t_readout_2nd=195+t_op+t_depump
    ...
    ## Blue imaging
    Blue480(t_start_blue_imaging+t_op+t_depump,t_blue_exposure,'r2')

    exp.camera.pulse_length=t_blue_exposure # Changes HSDIO pulse width to control exposure
    t_readout_3rd=205+t_op+t_depump
