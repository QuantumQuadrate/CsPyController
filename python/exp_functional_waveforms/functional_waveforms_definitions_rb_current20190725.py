...
def readoutduty(t,duration,dutyFORT):
    ...
    ## HEY!!! THIS VERSION OF READOUT TAKES A THIRD ARG, 'duty_RO_FORT', which
    ## sets the duty FORT pulse duty cycle. July 2019
...
## Initilization
    ...
    ############################################################################
    ## YO. Note that i'm using a different RO function in the 1st RO - preston
    ############################################################################

    # compare old  shims to new zero field setting
    # if alignment_loop == 0:
    #     pass
    # if alignment_loop == 1:
    #     shimX_RO = -0.3337
    ...
    ## preston's readout function
    # duty = .69
    duty = alignment_loop
    readoutduty(t_start+t_LAS_leadtime,t_leadtime+t_readoutduration+abs(t_LAS_leadtime),duty)
