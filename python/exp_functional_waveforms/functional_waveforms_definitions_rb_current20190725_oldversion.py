...
def readout(t,duration): # the standard RO function
    """Image atom in FORT."""
    ...
    # CAREFUL. Phases below are described correctly depending on the phase
    # profile used.

    # phases = [[0.18, 0.49],[0.69, 0.91]] # wide MOT, narrow FORT
    # phases = [[0.18, 0.4], [0.59+t_gap, 0.9-t_gap]] # variable width
    # phases = [[0.18, 0.4], [0.59+t_gap, 0.9+t_gap]] # wide, variable overall
    # phase
...
def readout2(t,duration):  # used primarily for PGC phase
    """Image atom in FORT."""
    # Note that camera trigger is not included here. Exposure control needs to be done indepdently
    # Also magnetic field control is done sepearately.
    ...
    print(t + cycles*period_ms/2)
    channels = [my_MOT_SW_channel, my_FORT_SW_channel]
    # phases = [[0.61, 0.92],[0.4, 0.62]] # July 2019

    offgap = 5*26.24e-9/(period_ms*1e-3)  # 5 gamma between MOT off, FORT on
    # phases = [[0.62-offgap, 0.92], [0.4, 0.62]]  # early August 2019
    phases = [[0.29, 0.99], [0.48, 0.78]]  #

    # For doing PGC after RO1
    # profiles = [
    #     [1, 0, 1]
    #     ,[0, 1, 0]
    # ]
    ...
    profiles = [
        [0, 1, 0],  # MOT: starts off, then cycle on, off
        [0, 1, 0]  # [1, 0, 1]   # FORT: starts on, then cycle off, on
    ]
...
def readoutduty(t,duration,dutyFORT):
    """Image atom in FORT. Now with tunable duty cycles :) """
    ## THIS VERSION OF READOUT TAKES A THIRD ARG, 'duty_RO_FORT', which
    ## sets the FORT pulse duty cycle. July 2019
    ...
    # set duty cycle for FORT num_of_pulses.
    # for phases [phase1, phase2], duty = 1 - (phase2-phase1).
    # hard code phase2, then set phase1 dependent on specified duty
    phase2 = 0.9
    phase1 = dutyFORT + phase2 - 1
    phases = [[0.18, 0.4], [phase1,phase2]]
...
## Initilization
    ...
    readout(t_start+t_LAS_leadtime,t_readoutduration+t_extrareadout)
    ...
    #Doing chopping
    extension = t_extrareadout + t_PGC_duration+5
    if t_PGC_duration>0:
