...
def readout2(t,duration):
    ...
    label(t, 'readout')
    # chop FORT and MOT out of phase
    cycles = int(duration*1000*RO_chopfreq)
    period_ms = 0.001/RO_chopfreq
    label(t + period_ms, 'readout c1')
    label(t + cycles*period_ms/2, 'readout half')
    print(t + cycles*period_ms/2)
    channels = [ 3]
...
## Initilization
    ...
    ## FORT Transfer Phase
    exp.fort_aom_switch.profile(t_2DMOT_loading-10,'on')
    exp.fort_dds.profile(t_2DMOT_loading-10,'on')
    ...
    if t_PGC_duration>0:
        ...
        readout4(t_start,t_PGC_duration)
