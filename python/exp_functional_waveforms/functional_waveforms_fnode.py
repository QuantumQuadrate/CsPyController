...
def pgc(t, duration, no_chop=False, bphase='pgc', fphase='on', phase='pgc', chop_cooling=False):
    ...
    if duration > 0:
        if not no_chop:
            ...
            if chop_cooling:
                channels += [CS_D2_DDS, RB_D2_DDS]
                phases += [
                    [0.25+mot_timing_offset, 0.65+mot_timing_offset],
...
def expmnt(t, duration, fphase='on', drop=True, exp_type='fdrop'):
    """Experiment type."""
    label(t, 'exp start')
    # HSDIO(t, HSDIO_channels['scope_trig_1']['channel'], True)
    fdrop_transient = 0
    ...
    if drop and (duration > 0):
        FORT_DDS(t-fdrop_transient, 'off')

    if exp_type == 'blue_align':
        t = blue_align_expmnt(t, duration, fphase=fphase)
    else:
        t += duration

    if drop and (duration > 0):
        FORT_DDS(t+fdrop_transient, fphase)
    label(t, 'exp end')
    # HSDIO(t+0.01, HSDIO_channels['scope_trig_1']['channel'], False)
    return t


def blue_align_expmnt(t, duration, fphase='on'):
...
def fort_experiment():
    ...
    #t = expmnt(t, fort_drop_us/1000, fphase='low', exp_type='blue_align', drop=False)
    t = expmnt(t, fort_drop_us/1000, fphase='low', exp_type='fdrop')
    t = uwave_rotation(t, rb_uwave_time_ms, gap=rb_uwave_gap_time_ms, atom='rb', fort_phase='low')
    t += 0.1
    t = uwave_rotation(t, cs_uwave_time_ms, gap=cs_uwave_gap_time_ms, atom='cs', fort_phase='low')
    t += 0.1
    FORT_DDS(t, 'on')
