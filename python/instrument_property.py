...
class Prop(Atom):
    ...
    def __init__(self, name, experiment, description=''):
        ...
        self.properties = [
            'description']  # things that are evaluated (if they define evaluate()) and saved to xml.  This is a list of the STRING of variable names (i.e. 'enable', not just: enable)
        self.doNotSendToHardware = ['description']

    def evaluate(self):
        """
        This function goes through the properties list and evaluates any of them
        with their own evaluate() method. This function should be overridden in
        subclasses at the lowest level on the Prop tree.
        """
        ...
        if self.experiment.allow_evaluation:
            for p in self.properties:
                try:
                    ...
                    logger.warning('In Prop.evaluate for {}: Property {} '
                                   'does not exist.'.format(self.name, p))
                    continue
                if hasattr(o, 'evaluate'):
                    try:
                        o.evaluate()
                    except PauseError:
                        raise PauseError
                        ...
                        logger.exception(
                            'Error evaluating {} in {}.properties: {}'.format(
                                p, self.name, e))
                        raise PauseError

    def toHDF5(self, hdf_parent_node, name=None):
        """
        This function provides generic behavior to save a Prop as an HDF5 group.
        The choice of group has been made because a Prop in general can have
        subproperties, and using a dataset would limit this behavior. We go
        through the properties list.  If an item has its own toHDF5() method,
        that will be used. If not, then we will try to save it as a dataset.
        This will only work if it is a type that h5py recognizes. Else, it will
        be pickled using python's human readable pickling format. And saved to
        the HDF5 as a string dataset. We assume that settings files will only
        ever be accessed as read-only and so variable length strings are not
        bothered with. The node name is self.name, because nodes must have
        unique names (and so node type cannot be used as the name).
        """

        if name is None:
            # if no name suggestion is given then use self.name
            name = self.name
        ...
        try:
            ...
            logger.warning(
                'Incompatible object `{}` already exists in `{}`. Deleting the'
                ' old object.'.format(name, hdf_parent_node.name))
        ...
        for p in self.properties:
            ...
            try:
                ...
                logger.warning('In Prop.toHDF5() for class {}: item {} in'
                               ' properties list does not exist.'.format(name,
                                                                         p))
                continue
            ...
            if hasattr(o, 'toHDF5'):
                ...
                try:
                    ...
                    logger.exception(
                        'While trying {}.toHDF5() in Prop.toHDF5() in {}:'
                        ' {}'.format(p, name, e))
                    raise PauseError
            else:
                if p == 'version':
                    ...
                    try:
                        ...
                        try:
                            ...
                            # otherwise just pickle it
                            try:
                                logger.debug(
                                    "Preparing to pickle field name.property: "
                                    "`{}.{}`".format(name, p))
                                my_node[p] = pickle.dumps(o)
                            except RuntimeError:
                                logger.debug(
                                    "Preparing to overwrite field name.property"
                                    ": `{}.{}`".format(name, p))
    ...
    def fromHDF5(self, hdf):
        ...
        First, version tags are checked. If an object exists and has its own
        fromHDF5 method, that will be used.  Else, we will attempt to load it
        as a python pickle. If this fails we load it as whatever type the HDF5
        dataset is stored as.  (The later is preferable to the python pickle,
        but we must try the pickle first, to distinguish between pickles and raw
        strings. self is the object corresponding to the top level tag in the
        parameter hdf, and its children are what will be loaded here.
        """
        ...
        if 'version' in hdf.attrs:
            if hasattr(self, 'version'):
                if hdf.attrs['version'] != self.version:
                    logger.info(
                        'Current ' + self.name + ' version is ' + self.version + ', you are loading from version: ' +
                        hdf.attrs['version'])
            else:
                logger.info(
                    'Code object ' + self.name + ' has no version, but HDF5 node has version: ' +
                    hdf.attrs['version'])
        elif hasattr(self, 'version'):
            logger.info(
                'Code object ' + self.name + ' has version ' + self.version + ' but HDF5 node has no version tag.')
        ...
        for i in hdf.attrs:
            if i != 'version':
                ...
                if i not in self.properties:
                    logger.info(
                        'Prop.fromHDF5(): HDF5 has attribute: ' + i + ', but this is not in the ' + self.name + '.properties list.  It will not be loaded.\n')
                else:
                    ...
                    try:
                        ...
                        logger.info(
                            'in {} in Prop.fromHDF5(). Will attempt to load '
                            'attribute {} which was not previously defined in '
                            '{}.'.format(self.name, i, self.name))
                        exists = False
                    if exists and hasattr(var, 'fromHDF5'):
                        ...
                        try:
                            ...
                            logger.exception(
                                'While trying attribute {}.fromHDF5() in '
                                'Prop.fromHDF5() in {}: {}'.format(i,
                                                                   self.name,
                                                                   e))
                            # this is an error, but we will not pass it on,
                            # in order to finish loading
                        ...
                        try:
                            ...
                            try:
                                x = hdf.attrs[i]
                                if exists:
                                    # mitigate the fact that atom cannot handle
                                    # numpy bool and int types
                                    if type(var) == bool:
                                        x = bool(x)
                                    elif type(var) == int:
                                        x = int(x)
                                ...
                                logger.warning(
                                    'Exception trying to load value for HDF5 '
                                    'attribute {} in {}.fromHDF5(): {}'.format(
                                        i, self.name, e))
                                # this is an error, but we will not pass it on,
                                # in order to finish loading
                                continue
                        try:
                            ...
                            logger.exception(
                                'in {} in Prop.fromHDF5() while setting '
                                'variable {} in {}: {}'.format(self.name, i,
                                                               self.name, e))
                            # this is an error, but we will not pass it on,
                            # in order to finish loading
                            continue
        ...
        for i in hdf:
            ...
            if i not in self.properties:
                logger.info(
                    'Prop.fromHDF5(): HDF5 has item: ' + i + ', but this is not in the ' + self.name + '.properties list.  It will not be loaded.\n')
            else:
                # load in all other tags into variables
                try:
                    # identify the variable to be loaded
                    ...
                    logger.info(
                        'in ' + self.name + ' in Prop.fromHDF5().  Will attempt to load ' + i + ' which was not previously defined in ' + self.name + '.\n')
                    exists = False
                if exists and hasattr(var, 'fromHDF5'):
                    # set it using its own method
                    # this will preserve the instance identity
                    try:
                        ...
                        logger.exception(
                            'While trying ' + i + '.fromHDF5() in Prop.fromHDF5() in ' + self.name + '.')
                    ...
                    if isinstance(hdf[i], h5py._hl.dataset.Dataset):
                        ...
                        try:
                            ...
                            logger.warning(
                                'in ' + self.name + ' in Prop.fromHDF5() while setting variable ' + i + ' in ' + self.name + '\n' + str(
                                    e) + '\n')
                            # this is an error, but we will not pass it on, in order to finish loading
                            continue
                    elif isinstance(hdf[i], h5py._hl.group.Group):
                        logger.warning(
                            'Cannot load HDF5 Group ' + i + ' without an fromHDF5() method in ' + self.name)
                        # this is an error, but we will not pass it on, in order to finish loading
                        continue
                    else:
                        logger.warning(
                            'Cannot load HDF5 node {} which is of type {} in {}.fromHDF5()'.format(
                                i, type(i), self.name))
                        # this is an error, but we will not pass it on, in order to finish loading
                        continue
        return self
    ...
    def toHardware(self):
        ...
        try:
            ...
            logger.warning(
                'While in format() in Prop.toHardware() in ' + self.name + '.\n' + str(
                    e) + '\n')
            raise PauseError
    ...
    def HardwareProtocol(self, o, name):
        ...
        if hasattr(o, 'toHardware'):
            try:
                ...
                msg = 'In Prop.HardwareProtocol() for class {} while trying '
                '{}.toHardware. {}'
                logger.exception(msg.format(self.name, name, e))
                raise PauseError
        ...
        # else just give str(o)
            ...
            try:
                ...
                logger.warning(
                    'In str(' + name + ') in Prop.HardwareProtocol() for ' + self.name + '.\n' + str(
                        e) + '\n')
                raise PauseError
    ...
    def evaluate(self):
        ...
        if self.experiment.allow_evaluation:
            ...
            try:
                ...
                logger.warning(
                    'EvalProp {} has no experiment assigned in '
                    'evaluate().'.format(self.name))
                vars = {}

            # evaluate the 'function'
            try:
                value, valid = cs_evaluate.evalWithDict(self.function,
                                                        varDict=vars)
            except Exception as e:
                logger.error(
                    'Error in EvalProp.evaluate() while evaluating property {}, {}, {}\n{}\n'.format(
                        self.name, self.description, self.function, e))
            ...
            try:
                ...
                logger.error(
                    'Exception in EvalProp.evaluate() in ' + self.name + '.\ndescription: ' + self.description + '\nfunction: ' + self.function + '\n' + str(
                        e) + '\n')
                self.set_gui({'valid': False, 'valueStr': ''})
                raise PauseError
...
class RangeProp(EvalProp):
    ...
    def __init__(self, name, experiment, description='', function='', low=None,
                 high=None):
        if low is not None:
            self.low = low
            self.hasLow = True
    ...
    def value_changed(self, changed):
        if self.hasLow:
            if self.low > changed['value']:
                raise TypeError(
                    'Attempt to assign {} to {}RangeProp {} but the minimum value is {}'.format(
                        changed['value'], self.numberType, self.name, self.low))
        if self.hasHigh:
            if changed['value'] > self.high:
                raise TypeError(
                    'Attempt to assign {} to {}RangeProp {} but the maximum value is {}'.format(
                        changed['value'], self.numberType, self.name,
                        self.high))
    ...
    def set_placeholder(self, changed):
        if self.hasLow:
            if self.hasHigh:
                self.placeholder = '{} < {} < {}'.format(self.low,
                                                         self.numberType,
                                                         self.high)
            else:
...
class EnumProp(EvalProp):
    ...
    def value_changed(self, changed):
        if not (changed['value'] in self.allowedValues):
            raise TypeError(
                'Attempt to assign {} to EnumProp {} but the only allowed values are: {}'.format(
                    changed['value'], self.name, self.allowedValues))
...
class ListProp(Prop):
    ...
    def refreshGUI(self):
        ...
        if self.gui is not None:
            try:
                ...
                logger.debug('ListProp call to refreshGUI encountered error:'
                             ' {}'.format(e))
    ...
    def getNextAvailableName(self):
        ...
        count = len(
            self.listProperty)  # start naming after current length, so this will go faster
    ...
    def add(self):
        new = self.listElementType(self.getNextAvailableName(), self.experiment,
                                   **self.listElementKwargs)
    ...
    def evaluate(self):
        if self.experiment.allow_evaluation:
            ...
            for i, o in enumerate(self.listProperty):
                if hasattr(o,
                           'evaluate'):  # check if it has an evaluate method.  If not, do nothing.
                    try:
                        o.evaluate()  # evaluate it
                    except PauseError:
                        raise PauseError
                    except Exception as e:
                        logger.exception('Evaluating list item ' + str(
                            i) + ' ' + o.name + ' in ListProp.evaluate() in ' + self.name + '.')
                        raise PauseError
        self.refreshGUI()

    def toHDF5(self, hdf, name=None):
        """
        ListProp has a special toHDF5 method because we do not save any of the
        normal properties for a listProp. It would be confusing to do so, as
        that is not what a ListProp is for.
        """
        ...
        for i, o in enumerate(self.listProperty):
            try:
                ...
                # using the given name didn't work, try again with an iterative
                # name
                try:
                    ...
                    logger.exception(
                        'Uncaught exception on list item {} in '
                        'ListProp.toHDF5item() in {}...{}'.format(i, self.name,
                                                                  e))
                    raise PauseError
        return list_node
    ...
    def toHDF5item(self, list_node, name, o):
        ...
        if hasattr(o, 'toHDF5'):
            ...
            try:
                ...
                logger.warning(
                    'While trying toHDF5() on list item {} in ListProp.toHDF5() in {}.'.format(
                        name, self.name))
                raise PauseError
        else:
            # try to save it as an attribute, then as a dataset. If that fails,
            # save its pickle
            try:
                # if it of a known well-behaved type, just go ahead and save to
                # HDF5 attrs (more efficient than dataset)
                list_node.attrs[name] = o
            except Exception:
                # if it is an array, saving to attrs will fail, but we can (and
                # should) save it as a dataset
                try:
                    list_node[name] = o
                except Exception:
                    # else just pickle it
                    try:
                        ...
                        logger.exception(
                            'While picking list item {} in ListProp.toHDF5() in'
                        )
                        raise PauseError

    def fromHDF5(self, hdf):
        """
        ListProp has a special fromHDF5 method because we do not try to load any
        of the normal properties. We only load things into the listProperty.
        """
        ...
        try:
            self.listProperty = [
                self.listElementType(
                    i, self.experiment, **self.listElementKwargs).fromHDF5(
                    hdf[i]) for i in hdf]
        except PauseError:
            raise PauseError
        except Exception as e:
            logger.warning(
                'in {} in ListProp.fromHDF5() for hdf node {}.'.format(
                    self.name, hdf.name))
            raise PauseError
        self.refreshGUI()
        return self
    ...
    def toHardware(self):
        ...
        for i, o in enumerate(self.listProperty):
            # give the index number as the XML tag, this will only be used if
            # the item does not have its own toHardware()
...
class Numpy1DProp(Prop):
    ...
    def __init__(self, name, experiment, description='', dtype=float,
                 hdf_dtype=float, zero=None):
    ...
    def toHDF5(self, hdf, name=None):
        try:
            hdf.create_dataset(self.name, data=self.array,
                               dtype=self.hdf_dtype)  # , compression="gzip", chunks=True)
        except Exception as e:
            logger.exception(
                'While trying to create dataset in Numpy1DProp.toHDF5() in ' + self.name + '.')
            raise PauseError
...
class Numpy2DProp(Prop):
    ...
    def toHDF5(self, hdf, name=None):
        try:
            ...
            logger.exception(
                'While trying to create dataset in Numpy2DProp.toHDF5() in ' + self.name + '.')
            raise PauseError
    ...
    def toHardware(self):
        try:
            ...
            logger.warning(
                'Exception in Numpy2DProp.toHardware() in ' + self.name + ' .\n' + str(
                    e))
            raise PauseError
